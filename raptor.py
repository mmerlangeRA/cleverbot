...
def embed_texts(texts: List[str],embeddingsModel:Embeddings) -> List[List[float]]:
    """Embed texts using OpenAIEmbeddings."""
    logging.info(f"Embedding {len(texts)} texts")
    return embeddingsModel.embed_documents([extract_text(text) for text in texts])
...
def summarize_texts(texts: List[str],llm:BaseChatModel) -> str:
...
def build_raptor_tree(texts: List[str], llm:BaseChatModel,embeddingsModel:Embeddings, max_levels: int = 3) -> Dict[int, pd.DataFrame]:
    ...
    for level in range(1, max_levels + 1):
        ...
        embeddings = embed_texts(current_texts,embeddingsModel=embeddingsModel)
        ...
        if len(current_texts) <= 1:
            results[level] = pd.DataFrame({
                'text': current_texts,
                'embedding': embed_texts(current_texts,embeddingsModel=embeddingsModel),
                'cluster': [0],
                'metadata': current_metadata
            })
...
def build_vectorstore(tree_results: Dict[int, pd.DataFrame],vectorStore: VectorStore) -> VectorStore:
    ...
    return vectorStore.from_documents(documents, embeddingsModel)

def build_faiss_vectorstore(tree_results: Dict[int, pd.DataFrame]) -> FAISS:
    ...
    return FAISS.from_documents(documents, embeddingsModel)

def tree_traversal_retrieval(query: str, vectorstore: VectorStore,embeddingsModel:Embeddings, k: int = 3) -> List[Document]:
    """Perform tree traversal retrieval."""
    query_embedding = embeddingsModel.embed_query(query)
...
def create_retriever(vectorstore: VectorStore,llm:BaseChatModel) -> ContextualCompressionRetriever:
...
def raptor_query(query: str, retriever: ContextualCompressionRetriever, llm:BaseChatModel, max_level: int) -> Dict[str, Any]:
...
if __name__ == "__main__":
    ...
    tree_results = build_raptor_tree(texts,llm,embeddingsModel)
    
    # Build vectorstore
    vectorstore = build_vectorstore(tree_results,vectorstore)
    # Create retriever
    retriever = create_retriever(vectorstore,llm)
    # Run the pipeline
    max_level = 3  # Adjust based on your tree depth
    query = "What is the greenhouse effect?"
    result = raptor_query(query, retriever, llm,max_level)
